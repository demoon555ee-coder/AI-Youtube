...
async def analyze(channel_id: str, payload: AnalyzeRequest, db: AsyncSession = Depends(get_db)):
    try:
...
async def decisions(channel_id: str, limit: int = Query(default=50, ge=1, le=100), db: AsyncSession = Depends(get_db)):
    try:
...
async def runs(channel_id: str, limit: int = Query(default=20, ge=1, le=100), db: AsyncSession = Depends(get_db)):
    try:
