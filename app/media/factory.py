...
def get_visual_provider(provider_name: str | None = None, config: dict | None = None) -> VisualAssetProvider:
    ...
    if name == "runway" or kind == "runway":
        api_key = os.getenv(str(cfg.get("api_key_env", "")), "") if cfg.get("api_key_env") else str(cfg.get("api_key") or settings.runway_api_key or settings.video_api_key or os.getenv("RUNWAY_API_KEY", "") or os.getenv("RUNWAYML_API_SECRET", "") or os.getenv("RUNWAY_API_SECRET", "") or os.getenv("RUNWAY_API", "") or os.getenv("Runway_API", ""))
