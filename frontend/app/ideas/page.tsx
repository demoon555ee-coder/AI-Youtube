...
export default function IdeasPage() {
  ...
  return <Shell>
    ...
    <div className="panel" style={{marginBottom:16}}><div className="cardTitle"><div><h2>Generate ideas</h2><p className="sub">Use an optional topic seed or let the strategy engine explore the niche.</p></div></div><div className="formRow"><div className="field"><label>Topic seed</label><input className="input" value={seed} onChange={e => setSeed(e.target.value)} placeholder="e.g. AI agents in 2027" /></div><div className="field"><label>Strategy goal</label><select className="select" value={goal} onChange={e => setGoal(e.target.value)}><option value="growth">Growth</option><option value="authority">Authority</option><option value="monetization">Monetization</option><option value="balanced">Balanced</option></select></div></div><div style={{marginTop:12}}><button className="btn primary" disabled={busy || !channelId} onClick={() => void generate()}>{busy ? "Working…" : "Generate 10 ideas"}</button></div></div>
