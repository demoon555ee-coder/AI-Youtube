...
- Google login callback failures (24.09.2026): VERIFIED по безопасному типу исключения в локальном API log. Сначала OAuthlib блокировал HTTP `localhost` (`InsecureTransportError`); разрешён HTTP только для loopback callback в development/test, production/staging отклоняют его. Следующий callback выявил scope mismatch: OAuth URL запрашивал короткие `email`/`profile`, а Google вернул канонические `userinfo.email`/`userinfo.profile`; OAuthlib правильно отклонил изменившийся scope через `Warning`. Исправлено: запрашивать канонические URI и оставить проверку scope включённой. Затем живой OAuth прошёл полностью: создана cookie session, onboarding загрузился с одним подключённым каналом `UCkQ5E8OpFWCPyRGhXnCK2zQ`. ID, показанный для аккаунта Google, отличается от целевого ID владельца `UCPIrnCahnAkGRgBdYONPZJQ`, поэтому связь целевого FinanceCompass канала ещё не подтверждена. Пустой список каналов допустим, сбой optional lookup не откатывает Google login.
